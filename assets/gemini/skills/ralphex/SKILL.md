...
- options:
  - label: "Full (Recommended)"
    description: "Task execution + Gemini review + Codex loop + final Gemini review"
  - label: "Review"
    description: "Skip tasks, run full review pipeline (Gemini + Codex + Gemini)"
  - label: "Codex-only"
    description: "Skip tasks and first Gemini review, run only Codex loop"
...
- Report current phase from progress file:
  ...
  - "review pass 1/2" → Gemini Review phase
...
## Nested Gemini CLI Sessions

ralphex automatically strips the `GEMINICODE` env var from child processes, allowing it to run from inside Gemini CLI. If the nested session error is somehow encountered, ralphex detects it via error pattern matching and exits gracefully instead of looping.
