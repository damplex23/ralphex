#!/usr/bin/env bash
# codex-as-gemini.sh - wraps codex CLI to produce Gemini-compatible stream-json output.
#
# this script translates codex JSONL events into the Gemini stream-json format
# that ralphex's GeminiExecutor can parse, allowing codex to be used as a drop-in
# replacement for gemini in task and review phases.
#
# config example (~/.config/ralphex/config or .ralphex/config):
#   gemini_command = /path/to/codex-as-gemini.sh
#   gemini_args =
#
...
# run codex with JSON output, translate events to gemini stream-json format.
