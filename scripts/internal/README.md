...
- **init-docker.sh** - Docker container init script. Copies Gemini and Codex credentials from mounted volumes into the app user's home directory. Run automatically by the base image on container start.
...
- **update-plugin-version.sh** - Updates version in `.gemini-plugin/plugin.json` and `marketplace.json`. Called by goreleaser as a pre-release hook.
