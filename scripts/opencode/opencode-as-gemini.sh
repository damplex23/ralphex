#!/usr/bin/env bash
# opencode-as-gemini.sh - wraps OpenCode CLI to produce Gemini-compatible stream-json output.
#
# this script translates OpenCode JSONL events into the Gemini stream-json format
# that ralphex's GeminiExecutor can parse, allowing OpenCode to be used as a drop-in
# replacement for gemini in task and review phases.
#
# config example (~/.config/ralphex/config or .ralphex/config):
#   gemini_command = /path/to/opencode-as-gemini.sh
#   gemini_args =
#
...
# enable auto-allow permissions for autonomous execution (equivalent to gemini's
...
# opencode's system prompt lacks Gemini CLI's "do not restate what the user said"
...
# run opencode with JSON output, translate events to gemini stream-json format.
