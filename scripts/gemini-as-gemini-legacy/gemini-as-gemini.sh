#!/usr/bin/env bash
# gemini-as-gemini.sh - wraps Gemini CLI to produce Gemini-compatible stream-json output.
#
# this script translates Gemini CLI plain-text output into the Gemini stream-json format
# that ralphex's GeminiExecutor can parse, allowing Gemini to be used as a drop-in
# replacement for gemini in task and review phases.
#
# config example (~/.config/ralphex/config or .ralphex/config):
#   gemini_command = /path/to/gemini-as-gemini.sh
#   gemini_args =
#
