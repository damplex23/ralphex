...
Wraps GitHub Copilot CLI to produce Gemini-compatible `stream-json` output, allowing Copilot to replace Gemini CLI in ralphex task, review, and plan phases through the existing `gemini_command` / `gemini_args` path.
...
Setting `gemini_args` to empty is optional. ralphex may still pass default Gemini flags because of config fallback behavior, and the wrapper ignores unknown flags gracefully.
...
```bash
bash scripts/copilot-as-gemini/copilot-as-gemini_test.sh
bash scripts/copilot-as-gemini/copilot-as-gemini_docs_test.sh
```
