#!/usr/bin/env bash
# copilot-as-gemini.sh - wraps GitHub Copilot CLI to produce Gemini-compatible stream-json output.
#
# this script translates Copilot CLI JSONL events into the Gemini stream-json format
# that ralphex's GeminiExecutor can parse, allowing copilot to be used as a drop-in
# replacement for gemini in task, review, and plan phases.
#
# config example (~/.config/ralphex/config or .ralphex/config):
#   gemini_command = /path/to/copilot-as-gemini.sh
#   gemini_args =
#
