#!/usr/bin/env bash
# agy-as-gemini.sh - wraps agy (Antigravity) CLI to produce Gemini-compatible stream-json output.
#
# this script translates agy plain-text output into the Gemini stream-json format
# that ralphex's GeminiExecutor can parse, allowing agy to be used as a drop-in
# replacement for gemini in task and review phases.
#
# config example (~/.config/ralphex/config or .ralphex/config):
#   gemini_command = /path/to/agy-as-gemini.sh
#   gemini_args =
#
