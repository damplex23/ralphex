...
## Overview
Add an included wrapper around GitHub Copilot CLI so it can replace Gemini CLI in ralphex task and review phases through the existing `gemini_command` / `gemini_args` integration path. Keep the feature in the wrapper-and-docs layer: no new Go executor, no new config keys, and no dedicated `external_review_tool = copilot` path unless the existing wrapper mechanism proves insufficient.

## Context
- Files involved:
  - Create: `scripts/copilot-as-gemini/copilot-as-gemini.sh`
  - Create: `scripts/copilot-as-gemini/copilot-as-gemini_test.sh`
  - Create: `scripts/copilot-as-gemini/README.md`
  - Modify: `docs/custom-providers.md`
  - Modify: `README.md`
  - Modify: `GEMINI.md`
- Related patterns:
  - `scripts/codex-as-gemini/`
  - `scripts/gemini-as-gemini/`
  - `scripts/opencode/`
  - `pkg/config/defaults/prompts/review_first.txt`
  - `pkg/config/defaults/prompts/review_second.txt`
  - `docs/plans/completed/2026-03-13-gemini-as-gemini.md`
- Dependencies:
...
- Existing integration path:
  - `gemini_command` / `gemini_args` already supports alternative CLIs that emit Gemini-compatible stream-json, so this should be implemented as a wrapper script, not new executor/config plumbing
- Key differences from current wrappers:
  ...
  - Review prompts are written in Gemini-specific “Task tool” language, so the wrapper may need a review adapter even though Copilot supports agentic workflows
...
- [x] Create `scripts/copilot-as-gemini/copilot-as-gemini.sh` following the existing wrapper layout and header-comment conventions
- [x] Read the prompt from stdin with `-p` fallback, and ignore unknown Gemini flags gracefully so the wrapper works under existing `gemini_args`
...
- [x] Translate Copilot JSONL text/completion events into Gemini-compatible `content_block_delta` and `result` events, preserving `<<<RALPHEX:...>>>` signals verbatim
...
- [x] Detect review prompts (`<<<RALPHEX:REVIEW_DONE>>>`) and prepend a Copilot-specific adapter that translates the Gemini “Task tool” instructions used in `review_first.txt` and `review_second.txt` into Copilot-compatible agent/subagent behavior
- [x] Create `scripts/copilot-as-gemini/copilot-as-gemini_test.sh` with a mock `copilot` binary that captures argv and stdin and emits configurable stdout, stderr, and exit codes
...
- [x] Run `bash scripts/copilot-as-gemini/copilot-as-gemini_test.sh`
...
- [x] Create `scripts/copilot-as-gemini/README.md` with setup instructions, config snippet, requirements, auth options, native Copilot env vars, and the wrapper test command
...
- [x] Update `GEMINI.md` script inventory if the new `scripts/copilot-as-gemini/` directory should be reflected there
...
**Files:**
- Verify: `scripts/copilot-as-gemini/copilot-as-gemini.sh`
- Verify: `scripts/copilot-as-gemini/copilot-as-gemini_test.sh`
...
- [x] Verify `scripts/copilot-as-gemini/copilot-as-gemini.sh` and `scripts/copilot-as-gemini/copilot-as-gemini_test.sh` are executable
- [x] Verify test coverage meets 80%+ (86.2% total via `make test`)

### Task 4: Update documentation

- [x] Confirm `README.md`, `docs/custom-providers.md`, and `GEMINI.md` all use the final wrapper path and naming consistently
