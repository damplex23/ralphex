...
- Project: ralphex (Go CLI for autonomous Gemini CLI plan execution).
- Existing ralphex skills live under `assets/gemini/skills/<name>/SKILL.md` with a flat symlink `assets/gemini/<name>.md` pointing to the same file. Plugin reads via `.gemini-plugin/plugin.json` `"skills": "./assets/gemini/skills/"`.
...
- Plugin version is in two places that must stay aligned: `.gemini-plugin/plugin.json` and `.gemini-plugin/marketplace.json`. Currently both at 0.18.0.
- llms.txt has an install-block convention for each skill (fetch URL, write target path).
- revdiff invoked via `~/.gemini/plugins/marketplaces/revdiff/.gemini-plugin/skills/revdiff/scripts/launch-revdiff.sh --wrap --only=<file>` for direct review (the wrapper `~/.gemini/scripts/draft-review.sh` runs a writing-style lint that misfires on plan content and writes a gh/glab approval marker the skill does not need).
...
- Plugin version bump is required because skill files under `assets/gemini/` are changing (per GEMINI.md "Plugin version" rule).

## Testing Strategy

- Unit tests: not applicable. The skill is markdown.
- Verification approach: after each task, re-read modified files and confirm content. After the final task, manually trigger the skill in a fresh Gemini session with a sample free-form markdown plan and confirm the conversion produces a valid ralphex-format plan that satisfies `pkg/plan/parse.go`'s `taskHeaderPattern`.
...
- Implementation Steps (`[ ]` checkboxes): write the SKILL.md content, create the symlink, add the llms.txt install block, update GEMINI.md, bump plugin version files. All achievable inside this repo.
- Post-Completion (no checkboxes): manual smoke test of the skill in a fresh Gemini session against a sample source plan, confirming the converted plan parses correctly with ralphex.
...
**Files:**
- Create: `assets/gemini/skills/ralphex-adopt/SKILL.md`

- [x] create directory `assets/gemini/skills/ralphex-adopt/`
...
- [x] write Step 5 (revdiff review loop) calling `~/.gemini/plugins/marketplaces/revdiff/.gemini-plugin/skills/revdiff/scripts/launch-revdiff.sh --wrap --only=<draft>` directly with the draft-review.sh bypass note, and the in-chat AskUser fallback
...
### Task 2: Wire skill into project (symlink, llms.txt, GEMINI.md)

**Files:**
- Create: `assets/gemini/ralphex-adopt.md` (symlink)
- Modify: `llms.txt`
- Modify: `GEMINI.md`

- [x] create symlink `assets/gemini/ralphex-adopt.md` pointing to `./skills/ralphex-adopt/SKILL.md` using `ln -s` (matches the existing three ralphex skills)
- [x] verify symlink resolves and shows the SKILL.md content via `cat assets/gemini/ralphex-adopt.md`
- [x] add an install block to `llms.txt` after the `/ralphex-update Skill` section, mirroring its structure: section header `### /ralphex-adopt Skill`, brief description of what the skill does, and the two install steps (fetch from `https://ralphex.com/assets/gemini/ralphex-adopt.md`, write to `~/.gemini/commands/ralphex-adopt.md`)
...
- [x] check `GEMINI.md` for the existing skill-mention pattern (`/ralphex-update` is mentioned at line near "Use `/ralphex-update` skill for smart merging..."); if a similar one-line mention fits, add `/ralphex-adopt` for plan conversion in the same vicinity
...
- Modify: `.gemini-plugin/marketplace.json`

- [x] bump `.gemini-plugin/plugin.json` `version` from `0.18.0` to `0.19.0` (minor: new skill addition, additive change)
- [x] bump `.gemini-plugin/marketplace.json` matching plugin entry from `0.18.0` to `0.19.0`
- [x] verify both files have matching version values via `grep -n version .gemini-plugin/plugin.json .gemini-plugin/marketplace.json`
- [x] verify both files are still valid JSON via `jq . .gemini-plugin/plugin.json` and `jq . .gemini-plugin/marketplace.json`

### Task 4: Verify acceptance criteria

- [x] verify `assets/gemini/skills/ralphex-adopt/SKILL.md` exists, has valid YAML frontmatter, and covers all six step-flow steps end-to-end
- [x] verify the symlink at `assets/gemini/ralphex-adopt.md` resolves to the SKILL.md (use `readlink` and `cat`)
- [x] verify `llms.txt` contains the new `/ralphex-adopt Skill` install block and updated skill-list line
- [x] verify `GEMINI.md` mentions `/ralphex-adopt` if a matching pattern was found in Task 2
- [x] verify both `.gemini-plugin/plugin.json` and `.gemini-plugin/marketplace.json` have version `0.19.0` and parse as valid JSON
...
- Trigger the skill in a fresh Gemini CLI session by typing `/ralphex-adopt <some-source>` and confirm the description in the frontmatter activates the skill.
