# Idle Timeout for Gemini Sessions

## Overview

Add `--idle-timeout` flag that kills Gemini sessions when no output is received for a specified duration. Addresses #248 where Gemini's process hangs after completing work (no output, no exit). Unlike `--session-timeout` (fixed wall-clock limit), idle timeout resets on each output line and only fires when the session goes silent.

## Context

- `pkg/executor/executor.go` — `GeminiExecutor` struct, `Run()`, `parseStream()`, `readLines()` callback
...
- `GeminiExecutor.IdleTimeout time.Duration` — new field, zero = disabled
...
### Task 1: Add IdleTimeout to GeminiExecutor with closure-based timer in Run

**Files:**
- Modify: `pkg/executor/executor.go`
- Modify: `pkg/executor/executor_test.go`

- [x] add `IdleTimeout time.Duration` field to `GeminiExecutor` struct
...
- [x] in `New()` of `runner.go`, set `geminiExec.IdleTimeout = cfg.AppConfig.IdleTimeout`
...
- [x] verify `--idle-timeout 5m` kills a hanging gemini session after 5 minutes of no output
...
- [x] update GEMINI.md — add idle timeout to Key Patterns and Configuration sections
