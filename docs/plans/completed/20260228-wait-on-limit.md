...
- Introduces separate `gemini_limit_patterns` and `codex_limit_patterns` config options that can overlap with error patterns
...
- [x] add `LimitPatterns []string` field to `GeminiExecutor` struct
- [x] in `GeminiExecutor.Run()`: check limit patterns BEFORE error patterns using `matchPattern()` — if limit match, return `LimitPatternError`; otherwise fall through to existing error pattern check returning `PatternMatchError`
...
- [x] write tests for `GeminiExecutor.Run()` — limit pattern takes precedence over error pattern when same string matches both
...
- [x] add `GeminiLimitPatterns []string`, `CodexLimitPatterns []string`, `WaitOnLimit time.Duration` fields to `config.Config`/`Values`
- [x] add `WaitOnLimitSet bool` flag to distinguish explicit `0` from "not set" (same pattern as `CodexEnabledSet`)
- [x] parse `gemini_limit_patterns`, `codex_limit_patterns` as comma-separated strings in `values.go` (same as error patterns)
...
- [x] add default entries to `pkg/config/defaults/config`: `gemini_limit_patterns = You've hit your limit` and `codex_limit_patterns = Rate limit,quota exceeded` and `wait_on_limit =` (empty/disabled)
...
- [x] wire `GeminiLimitPatterns` to `GeminiExecutor.LimitPatterns` in runner creation (`createRunner` or `processor.New`)
...
- [x] update `runTaskPhase()` — wrap `r.gemini.Run()` call with `runWithLimitRetry()`
- [x] update `runGeminiReview()` — wrap `r.gemini.Run()` call with `runWithLimitRetry()`
- [x] update `runGeminiReviewLoop()` — wrap `r.gemini.Run()` call with `runWithLimitRetry()`
- [x] update `runExternalReviewLoop()` — wrap external tool `cfg.runReview()` call with `runWithLimitRetry()`
- [x] update `runExternalReviewLoop()` — wrap gemini eval `r.gemini.Run()` call with `runWithLimitRetry()`
- [x] update `runPlanCreation()` — wrap `r.gemini.Run()` call with `runWithLimitRetry()`
...
- [x] update GEMINI.md — document new config fields under Configuration section and Error Pattern Detection section
...
```
- accepts a `run` function (works for gemini, codex, custom executors)
...
- `gemini_limit_patterns` — comma-separated, same format as error patterns
- `codex_limit_patterns` — comma-separated, same format as error patterns

**Config defaults:**
- `gemini_limit_patterns = You've hit your limit`
- `codex_limit_patterns = Rate limit,quota exceeded`
- `wait_on_limit =` (empty, disabled by default)

**Log output on limit retry:**
```
rate limit detected: "You've hit your limit" in gemini output, waiting 1h0m0s before retry...
```

## Post-Completion

**Manual verification:**
- test with actual rate limit scenario (hit Gemini limit, verify wait+retry works)
