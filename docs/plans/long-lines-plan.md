...
Windows has an 8191-character command-line limit (cmd.exe). The GeminiExecutor already works around this by passing prompts via stdin instead of CLI arguments. Other executors and wrapper scripts still pass potentially unbounded prompt text as CLI arguments, which will fail on Windows and could fail on other platforms with very large prompts.

## Context

- `GeminiExecutor` (Go): already fixed — passes prompt via `cmd.Stdin`
...
The codex CLI's `exec` subcommand reads prompt from stdin when no positional prompt argument is given. Mirror the GeminiExecutor pattern:
...
- [ ] Update tests in `codex-as-gemini_test.sh` if they verify args

### Task 3: Fix gemini-as-gemini.sh to pipe prompt via stdin
...
- [ ] Update tests in `gemini-as-gemini_test.sh` if they verify args

### Task 4: Fix opencode wrapper scripts

Both `opencode-as-gemini.sh` and `opencode-review.sh` pass prompt as CLI arguments.

- [ ] In `opencode-as-gemini.sh`: remove prompt from args, pipe via stdin or use temp file
...
- [ ] Update GEMINI.md platform support section if needed (mention codex stdin fix)
