...
Autonomous plan execution with Gemini CLI - Go rewrite of ralph.py.

## LLM Documentation

See @llms.txt for usage instructions and Gemini CLI integration commands.
...
pkg/executor/       # gemini and codex CLI execution
...
scripts/codex-as-gemini/ # codex wrapper for Gemini-compatible output
scripts/copilot-as-gemini/ # GitHub Copilot CLI wrapper for Gemini-compatible output
scripts/gemini-as-gemini/ # gemini wrapper for Gemini-compatible output
...
- `--codex` is an executor switch (not a new pipeline mode): sets `cfg.Executor = config.ExecutorCodex` so task, both reviews, and finalize run through `CodexExecutor`(s) with `MultiAgent=true` (enables `features.multi_agent`, registers the `reviewer` agent for spawn_agent calls). Forces `cfg.ExternalReviewTool = "none"` (codex-reviewing-codex is weak-signal self-review). `--pass-gemini.md` (codex executor only) sets `CodexExecutor.PassGeminiMd = true`. The `Mode` enum is unchanged; the `Executors` struct uses role-named fields (`Task`/`Review`/`External`/`Custom`), and `buildCodexExecutors` wires one codex instance into both `Task` and `Review` when the resolved review model/effort matches task, or two distinct instances when they differ. Review prompts are shared with gemini — the `{{agent:<name>}}` expansion in `pkg/processor/prompts.go` reads `cfg.AppConfig.Executor` and emits `Use the invoke_agent tool` (gemini) or `spawn_agent(agent='reviewer', task='...')` (codex). Codex config is passed as additive `-c` overrides per invocation by `(*CodexExecutor).configOverrides()` in `pkg/executor/codex.go`, layered on top of the user's `~/.codex/config.toml` so user customizations are preserved. ralphex never writes to `~/.codex/`; for user-level GEMINI.md it prints a one-time hint to `ln -s ~/.gemini/GEMINI.md ~/.codex/AGENTS.md`
- Codex review-phase directives: `prependCodexReviewGuidance` (`pkg/processor/prompts.go`) injects a `=== Codex orchestration directives ===` block through `promptBuilder.FirstReviewPrompt` and `promptBuilder.SecondReviewPrompt` when `cfg.isCodexExecutor()` is true (no-op for gemini). Covers two codex multi_agent quirks: (a) spawn_agent must pass only `agent` and `task` — `fork_context=true` with explicit `agent_type` is rejected by the codex API; (b) on a `wait_agent` timeout for a sub-agent that died mid-tool-call, re-spawn that agent ONCE then proceed with partial results. Section-level injection works for embedded and customized review prompts alike; `phase.ReviewPhase` consumes the final prompts.
- Codex task-phase skill-conflict directive: `prependCodexTaskGuidance` (`pkg/processor/prompts.go`) injects the `=== Codex task-execution directives ===` block (`codexTaskGuidance`) through `promptBuilder.TaskPrompt` when `cfg.isCodexExecutor()` is true (no-op for gemini). `phase.TaskPhase` consumes the final prompt. It tells codex that ralphex's task prompt is authoritative and a conflicting auto-activated skill from `~/.codex/skills/` must not be followed. Deliberately generic (names no specific skill); a soft prompt-level mitigation, not a hard guard — codex 0.133.0 has no per-invocation skill-disable flag. Task-phase only
...
- `--plan-model`/`--task-model`/`--review-model` resolve per-phase model/effort. `plan_model` falls back to `task_model`; `review_model` falls back to `task_model`. Gemini mode injects `--model`/`--effort` into `gemini_command`. Codex mode: `ResolveCodexModelEffort` (`pkg/processor/executor_factory.go`) resolves the `model[:effort]` spec against `codex_model`/`codex_reasoning_effort` defaults; `buildCodexExecutors` builds a separate review `CodexExecutor` when review differs from task. `max` effort does not exist in codex — kept default, `maxDropped` reported, `codexModelBanner` / `codexPlanBanner` (`cmd/ralphex/main.go`) warns
...
Custom scripts instead of codex for external review (`external_review_tool = custom`, `custom_review_script`). Script gets the prompt file path as its single arg, outputs findings to stdout for Gemini to evaluate.
...
- `session_timeout`/`idle_timeout` (see Configuration): in default Gemini mode neither applies to external codex/custom review; under `--codex` `session_timeout` covers every executor call
...
### Alternative Providers for Gemini Phases

`gemini_command`/`gemini_args` replace Gemini CLI with any `stream-json`-compatible CLI. Included wrappers: `scripts/codex-as-gemini/codex-as-gemini.sh`, `scripts/copilot-as-gemini/copilot-as-gemini.sh`. Wrappers must ignore unknown flags gracefully (`*) shift ;;`) — default Gemini flags may still be passed via config fallback. See `docs/custom-providers.md`.
...
`scripts/ralphex-dk.sh` supports AWS Bedrock as a Gemini provider (`--gemini-provider bedrock` / `RALPHEX_GEMINI_PROVIDER`). See `docs/bedrock-setup.md`.

Key functions in `scripts/ralphex-dk.sh`:
- `get_gemini_provider()` - returns provider from CLI flag or env var
...
- Gemini explores codebase and asks clarifying questions
...
- When ready, Gemini emits PLAN_DRAFT signal with full plan content for user review
...
- If revised (manually or via interactive review), feedback is passed to Gemini for plan modifications
- Loop continues until user accepts and Gemini emits PLAN_READY signal
...
- **Windows:** builds and runs, but with limitations:
  ...
  - Prompts are passed to the gemini CLI via stdin (not `-p` flag) to avoid the cmd.exe 8191-character command-line limit
...
- `session_timeout` config option: per-session timeout (e.g., "30m"). Applies to gemini in default mode and to every executor call under `--codex` (task/review/finalize/eval); external codex/custom review in Gemini mode is not affected. Kills hanging sessions, continues to next iteration. Applied in `retryPolicy.runWithSessionTimeout` via `context.WithTimeout`, gated on `Executor==ExecutorCodex || toolName=="gemini"`. CLI flag `--session-timeout` takes precedence. Disabled by default
- `idle_timeout` config option: kills gemini/codex executor sessions when no output for a given duration (e.g., "5m"). Resets on each output line; only fires when the session goes silent. Implemented in `GeminiExecutor.Run()`/`CodexExecutor.Run()` via `time.AfterFunc`. Wired by `buildCodexExecutor` for first-class `--codex`; NOT by `buildExternalCodexExecutor`, so external codex review in default-gemini mode has no idle timeout. Custom external review unaffected. CLI flag `--idle-timeout` takes precedence. Disabled by default
...
- `preserve_gemini_api_key` config option / `--preserve-gemini-api-key` CLI flag: when true, `GEMINI_API_KEY` is passed through to the child gemini process (needed for API-key auth rather than OAuth/keychain). Default `false` strips the key. The merge sentinel `PreserveGeminiAPIKeySet` lives only on `Values` (load-bearing for local-overrides-global merge); `Config` carries the resolved bool. Plumbed: `Config.PreserveGeminiAPIKey` → `pkg/processor/executor_factory.go` → `GeminiExecutor.PreserveAPIKey` → `execGeminiRunner.preserveAPIKey` → `geminiChildEnv()` (`pkg/executor/executor.go`). When enabled, the startup banner emits `auth: GEMINI_API_KEY passthrough enabled`. `GEMINICODE` is always stripped regardless (prevents nested-session errors)
...
Configurable patterns detect rate limit and quota errors in gemini/codex output:
- `gemini_error_patterns` / `codex_error_patterns`: comma-separated error patterns (default strings in `llms.txt` and the embedded config). Codex phrases are tightened so review findings that *talk about* rate limiting do not trip a false positive
- Matching is case-insensitive substring search
- Whitespace is trimmed from each pattern
- For gemini: patterns checked against the last 10 text blocks (not full output) to avoid false positives when analysis text mentions rate limit phrases. Context cancellation paths bypass pattern checks
...
- `gemini_retry_patterns`: comma-separated transient Gemini/fya markers retried like executor timeouts. Default: `FYA_TRANSIENT_TIMEOUT,API Error: 529,API Error: 502,API Error: 503,API Error: 504`. The transient HTTP errors (529 Overloaded, 502/503/504 gateway) live here, not in `gemini_limit_patterns`: they are short-lived server hiccups, so they auto-retry without requiring `--wait` (500 is intentionally excluded — it can be a deterministic server failure, caught by the broad `API Error:` error pattern). Precedence is retry → limit → error, so a no-signal 529 matches the retry tier first; a 529 with a signal present falls through to the broad `API Error:` error pattern
...
- `gemini_limit_patterns` / `codex_limit_patterns`: comma-separated limit patterns (default strings in `llms.txt` and the embedded config)
...
- Patterns passed via `GeminiExecutor.ErrorPatterns`/`LimitPatterns`/`RetryPatterns` and `CodexExecutor.ErrorPatterns`/`LimitPatterns` (codex has no retry patterns)
...
- `{{agent:name}}` - expands to invoke_agent tool instructions for the named agent
...
- Alternatively, reference agents installed in your Gemini CLI directly in prompt files (like `qa-expert`, `go-smells-expert`)
...
Unit tests mock external calls. After ANY code changes, ask the user before running an e2e test with a toy project because it can take time and consume gemini/codex credits. Run it only after explicit approval to verify actual gemini/codex integration and output streaming.
...
3. Phase 2: first Gemini review
4. Phase 2.5: codex external review
5. Phase 3: second Gemini review
...
Unit tests don't verify actual codex/gemini integration or output formatting. The toy project test is the only way to verify streaming output works correctly.
...
- **Raw .md files**: SSG renders ALL `.md` files in `docs_dir` as HTML pages. To serve raw markdown (e.g., `assets/gemini/*.md` for Gemini CLI skills), copy them AFTER `zensical build` - see `prep_site` target in Makefile
...
- **Plugin version**: bump `.gemini-plugin/plugin.json` and `.gemini-plugin/marketplace.json` versions on release if skill files (`assets/gemini/`) changed since last plugin version bump
